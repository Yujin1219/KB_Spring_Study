...
public class MemberController {
    ...
    //    @Autowired  // 기존 프로젝트는 Autowired를 통해 객체를 주입한다.
    @Setter(onMethod_ = {@Autowired}) // setter를 통해 Bean 주입하는 방법
    private MemberService service;

    // RequestMapping : 가장 기본적으로 요청처리 가능한 어노테이션, get/post를 둘다 처리 가능
    @RequestMapping(value = {"/member/index.do", "/member"}, method = RequestMethod.GET)
    public String index(/* 이곳에 파라미터와 어노테이션으로 채울 수 있는 공간 */) {
        return "member/index";
    }
...
//    @GetMapping("/member/memberServlet.do")
    ...
    public String memberServlet(HttpServletRequest req, HttpServletResponse resp,
        ...
        // 세션 가져오는 방법 2 - Spring에서 session을 주입해주는 방법 ★★★★★
...
// - 현업에서는 map을 받아서 DB나 Mybatis로 넘겨 동적쿼리로만 인자를 활용하는 패턴 -> 추천X
    ...
    public String memberRestGet(Model model, @PathVariable String type, @PathVariable("id") String memberId) {
    ...
    @RequestMapping("/member/memberAddInfo.do") //get,post 둘 다 처리
    public void memberAddInfo(Writer writer,
                              ...
                              @CookieValue(value = "saceId", required = false) String sessionId
            ) throws IOException {
        writer.append("<html>");
        ...
        writer.append("</html>");
            ...
            public ModelAndView memberEnroll(ModelAndView model, MemberDTO member) {
                ...
                if(result > 0){
                    ...
                    model.setViewName("redirect:error.do"); // 에러 페이지로 리다이렉트
                }
                return model;
            }
