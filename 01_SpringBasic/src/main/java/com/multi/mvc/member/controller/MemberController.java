...
//    @Autowired // 기존 프로젝트는 대부분 Autowired를 통해 객체를 주입한다.
    ...
    // RequestMapping : 가장 기본적인 요청처리가 가능한 어노테이션, get/post를 둘 다 처리 가능
...
//    @GetMapping("/member/memberServlet.do")
    public String memberServlet(HttpServletRequest req, HttpServletResponse resp, HttpSession session){
        ...
        member.setDevLang(new ArrayList<>(Arrays.asList(req.getParameterValues("devLang"))));
        ...
        // 세션 가져오는 방법2 - Spring 에서 session을 주입해주는 방법 ★★★★★
    ...
    public String memberParams(Model model,
            @RequestParam(value = "id", defaultValue = "-", required = false) String memberId,  // ★required = false : 없으면 id 없으면 죽음
...
//            @RequestParam(value = "devLang", required = false) String[] devLang //배열도 가능!
        ...
        // addAttribute : key-value 값으로 인자를 넘기는 방법, request와 동일
